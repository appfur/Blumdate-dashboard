      ...
      try {
        ...
        if (data.status === 'success') {
          const validUsers = data.data.profiles
            .filter(profile => profile.userId && profile.userId._id && profile.userId.premium)
            .map(profile => ({
              ...
              subscriptionDate: profile.userId.createdAt ? new Date(profile.userId.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'N/A',
          ...
          <table className="w-full text-left border-collapse bg-white shadow-lg rounded-lg hidden md:table">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="py-3 px-4 border-b-2 border-gray-200 text-sm font-semibold text-gray-700">Subscription Status</th>
                <th className="py-3 px-4 border-b-2 border-gray-200 text-sm font-semibold text-gray-700">Email</th>
                <th className="py-3 px-4 border-b-2 border-gray-200 text-sm font-semibold text-gray-700">Subscription Date</th>
                ...
                >
                  ...
                  <td className="py-3 px-4 border-b border-gray-200 text-sm text-gray-600">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                      user.subscription.status === 'Active' ? 'text-purple-700 bg-purple-100' : 'text-red-700 bg-red-100'
                    }`}>
                      {user.subscription.status}
                    </span>
                  </td>
                  <td className="py-3 px-4 border-b border-gray-200 text-sm text-gray-600">{user.email}</td>
                  <td className="py-3 px-4 border-b border-gray-200 text-sm text-gray-600">{user.subscriptionDate}</td>
                  <td className="py-3 px-4 border-b border-gray-200 text-sm text-gray-600">
            ...
            {currentUsers.map((user) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="font-medium text-gray-700">Subscription Status</div>
                  ...
                  <div className="font-medium text-gray-700">Subscription Date</div>
                  <div className="text-gray-700">{user.subscriptionDate}</div>
                </div>
